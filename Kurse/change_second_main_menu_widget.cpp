...
void Change_second_main_menu_widget::reicive_new_row(int row)
{
    QString q = QString::number(row);
    ui->textEdit->setText(q);
    //сохраняем id
    //selected_id = id;
    //добавляем в List_Things
    /*QSqlTableModel *ListModel = new QSqlTableModel();
    ListModel->setTable("List_Things");
    ListModel->setEditStrategy(QSqlTableModel::OnManualSubmit);
    int size = ListModel->rowCount();
    ListModel->insertRow(size);*/
    //ListModel->setData(ListModel->index(size+1,1), "jc");
}
